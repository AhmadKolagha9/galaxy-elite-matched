...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${getBackendApiUrl()}/api/availability`, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    return NextResponse.json(
      { ok: false, error: apiErrorMessage(payload, 'Availability submission failed.') },
      { status: response.status }
    )
  }

  return NextResponse.json(
    payload ?? { ok: true, message: 'Availability profile securely logged. Pending Galaxy Elite administrative review.' },
    { status: response.status }
  )
}
