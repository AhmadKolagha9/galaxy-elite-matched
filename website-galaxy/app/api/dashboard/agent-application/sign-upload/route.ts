...
import { getBackendAuthToken, getBackendJwtPayloadFromCookie } from '@/lib/native-session'

export const runtime = 'nodejs'

const allowedTypes = new Set(['owner_id', 'broker_licence'])
const allowedMimeTypes = new Set(['application/pdf', 'image/png', 'image/jpeg'])

export async function POST(request: Request) {
  const token = await getBackendAuthToken()
  const payload = token ? await getBackendJwtPayloadFromCookie() : null
  ...
  const response = await fetch(`${getBackendApiUrl()}/api/upload/sign-url`, {
    ...
    body: JSON.stringify({
      ...
      mime_type: mimeType,
      file_size: fileSize,
      related_object_type: 'agent_application',
      related_object_id: payload.sub
    })
  })

  const responseBody = await parseApiResponse(response)
  if (!response.ok) return NextResponse.json({ ok: false, error: apiErrorMessage(responseBody, 'Could not create signed upload URL.') }, { status: response.status })
