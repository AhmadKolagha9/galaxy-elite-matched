...
import { archiveTaxonomyItem, controlErrorMessage, controlErrorStatus, saveTaxonomyItem } from '@/lib/control-api'
...
function payloadFromBody(id: string, body: Record<string, unknown>): TaxonomyMutationPayload | null {
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const payload = body ? payloadFromBody(id, body) : null
  ...
  try {
    ...
    return NextResponse.json({ ok: false, error: controlErrorMessage(error, 'Could not update taxonomy item.') }, { status: controlErrorStatus(error) })
  }
}
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const item = await archiveTaxonomyItem(id)
    return NextResponse.json({ ok: true, item })
  } catch (error) {
    return NextResponse.json({ ok: false, error: controlErrorMessage(error, 'Could not archive taxonomy item.') }, { status: controlErrorStatus(error) })
  }
}
