...
import { getBackendAdminCollections, getBackendAdminSummary, getBackendApprovedInterestCards, getBackendCollection, isBackendApiConfigured, updateBackendSubmissionDecision } from '@/lib/backend-api'
...
export async function getCollection(collection: CollectionKey): Promise<SubmissionRecord[]> {
  if (isBackendApiConfigured()) return getBackendCollection(collection)
...
export async function updateSubmissionDecision({
  ...
  if (isBackendApiConfigured()) {
    await updateBackendSubmissionDecision({ id, approvalStatus, publicStatus, verificationLevel, complianceNotes })
    return
  }
...
export async function getAllAdminCollections() {
  if (isBackendApiConfigured()) return getBackendAdminCollections()
...
export async function getAdminSummary() {
  if (isBackendApiConfigured()) return getBackendAdminSummary()
...
export async function getApprovedInterestCards(): Promise<InterestCardData[]> {
  if (isBackendApiConfigured()) return getBackendApprovedInterestCards()
